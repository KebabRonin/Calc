...
{
public:
	void imagine(double lungime, int nivel, CPunct &p, int d)
	{
		double lungimeDiagH = 0.0, lungimeLatura = 0.0, d1, d2;
		CVector v(1.0, 0.0);
		CPunct punctUrmator, start;

		if (nivel == 0)
		{
			lungimeDiagH = lungime * sqrt(2) / 6.0;
			lungimeLatura = lungime / 3.0;

			v.rotatie(135);
			start = v.getDest(p, lungimeDiagH);
			v.rotatie(-135);
		...
		else
		{
			...
			imagine(lungime, 0, p, d);

			for (int i = 0; i < 8; i++) 
			{
				punctUrmator = v.getDest(p, i % 2 ? d1 : d2);
				imagine(lungimeLatura, nivel - 1, punctUrmator, d);
    ...
    void deseneazaPatrat(double lungime) 
    {
        CVector v(1.0, 0.0);
        CPunct start(0 - lungime / 2.0, 0 + lungime / 2.0); 
    ...
    void patratInconjuratorCuGrafic(double lungime, int nivel, CPunct& p) 
    {
        double dimensiunePatrat = lungime; 
      
        deseneazaPatrat(dimensiunePatrat);


        imagine(lungime, nivel, p, 1);
    }
	...
	void afisare(double lungime, int nivel) 
	{
		...
		patratInconjuratorCuGrafic(lungime, nivel, p);
	}
};
...
{
public:
	void imagine(double lungime, int nivel, double factordiviziune, CPunct &p, CVector &v) 
	{
		CPunct p1, p2;
		if (nivel == 0)
		{
		...
		else 
		{
			...
			v.deseneaza(p, lungime);
			p1 = v.getDest(p, lungime);
			imagine(lungime * factordiviziune, nivel - 1, factordiviziune, p1, v);
...
{
public:
	void imagine(double lungime, int nivel, CPunct &p, CVector &v, int d) 
	{
		if (nivel == 0)
		{
			v.deseneaza(p, lungime);
...
void Display10() {
	Imagine3 img3;
	img3.afisare(0.8 / pow(2.0, (nivel - 1)), nivel);
